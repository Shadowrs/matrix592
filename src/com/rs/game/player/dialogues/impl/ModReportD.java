...
public class ModReportD extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		case OPTION_1:
			...
			target.message("You've been muted for 1 hour by " + player.getDisplayName() + ".");
			player.message("You've muted " + target.getDisplayName() + " for 1 hour.");
			break;
		case OPTION_2:
			target.setMuted(Utils.currentTimeMillis() + (48 * 60 * 60 * 1000));
			target.message("You've been muted for 48 hours by " + player.getDisplayName() + ".");
			player.message("You've muted " + target.getDisplayName() + " for 48 hours.");
			break;
		case OPTION_3:
			target.setBanned(Utils.currentTimeMillis() + (48 * 60 * 60 * 1000));
			target.getSession().close();
			player.message("You've banned " + target.getDisplayName() + " for 48 hours.");
			break;
		case OPTION_4:
			target.setPermBanned(true);
			target.getSession().close();
			player.message("You've banned " + target.getDisplayName() + "  permanently.");
			break;
		case OPTION_5:
			target.setPermBanned(true);
